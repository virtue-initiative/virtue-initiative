...
| Locked Password Value | Server  | `HPKE(DHKEM(X25519), HKDF-SHA256, AES-256-GCM)` (sealed for the owner only) | Owner            |
...
This key is recreated on a password reset. Changing the password from Settings
keeps the key. The browser decrypts it with the current password key, then
encrypts it again with the new one, so batches encrypted before the change stay
readable.
...
A [locked password](/help/web/locked-passwords) value is sealed once, for the
owner's own public key, using the same primitive as a batch key:

```
HPKE(DHKEM(X25519), HKDF-SHA256, AES-256-GCM)
```

Unlike a batch key, it's never wrapped for anyone else. Partners hold no key
that could open it, and have no endpoint that would let them try. The server
stores and returns the sealed value without ever being able to read it
itself.
...
Source: [`web/src/crypto.ts:L83-L110`](https://github.com/virtue-initiative/virtue-initiative/blob/main/web/src/crypto.ts#L83-L110)
...
Source: [`web/src/crypto.ts:L112-L124`](https://github.com/virtue-initiative/virtue-initiative/blob/main/web/src/crypto.ts#L112-L124)
...
Source: [`web/src/crypto.ts:L140-L151`](https://github.com/virtue-initiative/virtue-initiative/blob/main/web/src/crypto.ts#L140-L151)
...
Source: [`client/core/src/crypto.rs:L82-L105`](https://github.com/virtue-initiative/virtue-initiative/blob/main/client/core/src/crypto.rs#L82-L105)
...
Source: [`client/core/src/batch.rs:L14-L56`](https://github.com/virtue-initiative/virtue-initiative/blob/main/client/core/src/batch.rs#L14-L56)
