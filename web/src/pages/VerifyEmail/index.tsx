...
function hasRecentInflightVerification() {
  ...
  if (!Number.isFinite(startedAt) || Date.now() - startedAt > VERIFY_INFLIGHT_TTL_MS) {
    ...
    api
      ...
      .then(async (result) => {
        ...
        window.sessionStorage.setItem(
          ...
          JSON.stringify({
            message: isEmailChange ? 'Email changed successfully.' : 'Email verified successfully.',
            isError: false,
          }),
        );
        if (isEmailChange) {
        ...
        if (invite) {
          navigate(`/?partner_invite_token=${encodeURIComponent(invite)}`, true);
          return;
        }
        navigate('/', true);
      })
