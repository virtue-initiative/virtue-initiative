...
describe("decodeWebpDimensions", () => {
  it("parses VP8 lossy", () => {
    expect(decodeWebpDimensions(makeVP8(1920, 1080))).toEqual({ width: 1920, height: 1080 });
  });

  it("parses VP8L lossless", () => {
    expect(decodeWebpDimensions(makeVP8L(800, 600))).toEqual({ width: 800, height: 600 });
  });

  it("parses VP8X extended", () => {
    expect(decodeWebpDimensions(makeVP8X(3840, 2160))).toEqual({ width: 3840, height: 2160 });
  });
  ...
  it("returns undefined for truncated buffer (cut after WEBP)", () => {
    expect(decodeWebpDimensions(new Uint8Array([
  ...
  it("returns undefined for unknown chunk type", () => {
    ...
    buf[8] = 0x57; buf[9] = 0x45; buf[10] = 0x42; buf[11] = 0x50;
    // Chunk type "VP9 "
    buf[12] = 0x56; buf[13] = 0x50; buf[14] = 0x39; buf[15] = 0x20;
