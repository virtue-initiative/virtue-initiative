...
The server MUST NOT return credential or secret material from either presets or raw queries. At minimum the columns `password_hash`, `password_salt`, `encrypted_priv_key`, `wrapped_value`, `access_keys`, and any column whose name ends in `token_hash` MUST be dropped from the result by name. The server SHOULD render 16-byte BLOB values as UUID strings and other BLOBs as hex.
