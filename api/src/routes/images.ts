...
images.post('/', authenticate, async (c) => {
  ...
  const extension = content_type.split('/')[1] || 'jpg';
  const r2Key = `user/${userId}/images/${imageId}.${extension}`;
  ...
  const uploadUrl = await generateUploadUrl(c.env, r2Key, content_type);
