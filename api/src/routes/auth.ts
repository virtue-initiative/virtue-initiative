...
auth.post('/signup', async (c) => {
  ...
  const accessToken = await generateAccessToken(userId, c.env.JWT_SECRET, c.env.JWT_ACCESS_EXPIRY);
  const refreshToken = await generateRefreshToken(userId, c.env.JWT_SECRET, c.env.JWT_REFRESH_EXPIRY);
...
auth.post('/login', async (c) => {
  ...
  const accessToken = await generateAccessToken(user.id as string, c.env.JWT_SECRET, c.env.JWT_ACCESS_EXPIRY);
  const refreshToken = await generateRefreshToken(user.id as string, c.env.JWT_SECRET, c.env.JWT_REFRESH_EXPIRY);
...
auth.post('/token', async (c) => {
  ...
  try {
    ...
    const accessToken = await generateAccessToken(payload.sub, c.env.JWT_SECRET, c.env.JWT_ACCESS_EXPIRY);
