...
export async function findUserByEmail(db: D1Database, email: string) {
  return db.prepare('SELECT id, email, password_hash FROM users WHERE email = ?').bind(email).first<{
    id: string;
    email: string;
...
export async function createUser(
  ...
  ).bind(id, email, passwordHash, name, createdAt).run();
}
...
export async function createDevice(
  ...
  return db.prepare(
    `INSERT INTO devices (id, user_id, name, platform, avg_interval_seconds, enabled, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, name, platform, avgIntervalSeconds, 1, createdAt).run();
}
...
export async function findDevice(db: D1Database, deviceId: string, userId: string) {
  ...
  ).bind(deviceId, userId).first<{ id: string }>();
}
...
export async function updateDevice(
  ...
  if (fields.interval_seconds !== undefined) { updates.push('avg_interval_seconds = ?'); params.push(fields.interval_seconds); }
  if (fields.enabled !== undefined) { updates.push('enabled = ?'); params.push(fields.enabled ? 1 : 0); }

  params.push(deviceId);
  return db.prepare(`UPDATE devices SET ${updates.join(', ')} WHERE id = ?`).bind(...params).run();
}
...
export async function createImage(
  ...
  ).bind(id, userId, deviceId, r2Key, sha256, contentType, sizeBytes, takenAt, createdAt).run();
}

export async function findImageById(db: D1Database, imageId: string) {
  return db.prepare('SELECT r2_key FROM images WHERE id = ?').bind(imageId).first<{ r2_key: string }>();
}
...
export async function createLog(
  ...
  return db.prepare(
    `INSERT INTO logs (id, user_id, device_id, image_id, type, metadata, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, userId, deviceId, imageId, type, metadata, createdAt).run();
}
...
export async function createPartner(
  ...
  return db.prepare(
    `INSERT INTO partners (id, user_id, partner_user_id, status, permissions, created_at, updated_at)
     VALUES (?, ?, ?, 'pending', ?, ?, ?)`
  ).bind(id, userId, partnerUserId, permissions, createdAt, createdAt).run();
}
...
export async function findPartnerByUsers(db: D1Database, userId: string, partnerUserId: string) {
  ...
  ).bind(userId, partnerUserId).first<{ id: string }>();
}
...
export async function findPartnerInvite(db: D1Database, partnerId: string, userId: string) {
  ...
  ).bind(partnerId, userId).first<{ id: string }>();
}
  ...
  const [owned, asPartner] = await Promise.all([
    db.prepare(
      `SELECT p.id, u.email as partner_email, p.status, p.permissions, p.created_at
       ...
       WHERE p.user_id = ?`
    ).bind(userId).all<{ id: string; partner_email: string; status: string; permissions: string; created_at: string }>(),
    db.prepare(
      `SELECT p.id, u.email as partner_email, p.status, p.permissions, p.created_at
...
export async function updatePartnerPermissions(
  ...
  ).bind(permissions, updatedAt, partnerId).run();
}
...
export async function getSettings(db: D1Database, userId: string) {
  return db.prepare('SELECT data FROM settings WHERE user_id = ?').bind(userId).first<{ data: string }>();
}

export async function saveSettings(db: D1Database, userId: string, data: string, updatedAt: string) {
  return db.prepare(
