...
const docPath = path.join(repoRoot, 'landing/src/content/help/developer/security.mdx');
...
const references = [
  {
    ...
    file: 'web/src/utils/api/crypto.ts',
    language: 'ts',
    startMarker: 'export async function derivePasswordMaterial(',
  },
  {
    title: 'Browser private-key wrapping',
    description: 'The web client encrypts the private key locally with AES-GCM before upload.',
    file: 'web/src/utils/api/crypto.ts',
    language: 'ts',
  ...
  },
  {
    ...
    description: "The web app generates the user keypair from the HPKE suite's X25519 KEM.",
    file: 'web/src/utils/api/crypto.ts',
    language: 'ts',
  ...
  },
  {
    ...
    description: 'The Rust client matches the browser flow with Argon2id and HKDF-SHA256("auth").',
  ...
  },
  {
    ...
    file: 'client/core/src/module/upload/batch.rs',
    language: 'rs',
    startMarker: '    pub fn build_upload(',
  },
  {
    title: 'Native client request signing',
    description:
    ...
    file: 'client/core/src/crypto.rs',
    language: 'rs',
    startMarker: 'pub fn sign_request(',
  },
  {
    title: 'Hash-server signature verification',
    description:
      "The hash-server verifies each signed POST /hash request's signature and timestamp, rejecting stale or replayed requests.",
    file: 'hash-server/src/auth.rs',
    language: 'rs',
    startMarker: 'pub fn verify_signature(',
  },
];
...
function renderSection(baseUrl, ref) {
  ...
  for (const reference of references) {
    ...
    const githubUrl = buildGitHubUrl(baseUrl, ref, reference.file, startLine, endLine);

    parts.push(`### ${reference.title}`);
    parts.push('');
    parts.push(reference.description);
    parts.push('');
    parts.push(`Source: [\`${reference.file}:L${startLine}-L${endLine}\`](${githubUrl})`);
    parts.push('');
